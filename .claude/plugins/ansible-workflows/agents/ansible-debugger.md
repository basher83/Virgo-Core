...
6. Hand off fixed code to ansible-validator for re-validation

**Required Skills to Load:**

Before debugging, load these skills using the Skill tool:

1. `ansible-workflows:ansible-fundamentals` - Core Ansible patterns and FQCN
2. `ansible-workflows:ansible-idempotency` - changed_when, check-before-create patterns
3. `ansible-workflows:ansible-error-handling` - block/rescue, failed_when, retries
4. `ansible-workflows:ansible-proxmox` - Proxmox-specific modules and CLI patterns
...
- [Optional improvements beyond the immediate fix]
- [Related best practices to consider]
```
